...
# Re-added the blink_led function
def blink_led(count):
    if not GPIO_SUPPORT:
        return
...
class MouseControlThread(threading.Thread):
    ...
    def run(self):
        ...
        # We need a reference size. This should ideally be the actual video resolution or a known max size.
        # Use the camera resolution for a more accurate mapping.
        VIDEO_WIDTH = RESOLUTION[0]
        VIDEO_HEIGHT = RESOLUTION[1]
        
        # Half of the video resolution for centered coordinates
        ...
        while self.running:
            ...
            # Map coordinates from -HALF_WIDTH to HALF_WIDTH and -HALF_HEIGHT to HALF_HEIGHT
            # to servo angle range
            target_pan_angle = (target_x / HALF_VIDEO_WIDTH) * PAN_RANGE
            target_tilt_angle = (target_y / HALF_VIDEO_HEIGHT) * TILT_RANGE
            
            # Clamp the angles to prevent the servos from going out of range
            ...
            # Gradually move servos to the target position
...
# Initialize and start the mouse control thread
mouse_control_thread = MouseControlThread()
if GPIO_SUPPORT:
    mouse_control_thread.start()

# --- Existing code for other functions (blink, cleanup, etc.) remains unchanged ---
